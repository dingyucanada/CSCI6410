...
- description: Brief summary of the patient's case.
- medical_specialty: The medical specialty related to the note (e.g., Surgery).
- sample_name: The clinical case or procedure name.
- transcription: Full free-text medical note.
...
analysis.data <- filtered.data %>%
  unnest_tokens(word, transcription) %>%
  mutate(word = stringr::str_replace_all(word, "[^[:alnum:]]", "")) %>%
...
library(dplyr)
# assuming tokenized.data.unigram has columns: note_id, medical_specialty, word
...
There are more unique unigrams in Surgery than Orthopedic and Radiology, reflecting more diverse language in surgical notes.



**3** How many unique bi-grams are there in each category without stop words and numbers?

The number of unique bigrams is highest in Surgery, followed by Orthopedic, then Radiology, which suggests more varied phrase usage in surgical documentation.

```{r}
tokenized.data.bigram <- analysis.data %>%
  ...
  dplyr::filter(
    ...
    !stringr::str_detect(word1, "[0-9]"),
    !stringr::str_detect(word2, "[0-9]")
  ) %>%
  dplyr::mutate(bigram = paste(word1, word2)) %>%
  dplyr::group_by(note_id, medical_specialty, bigram) %>%
  dplyr::summarise(count = n(), .groups = "drop")

str(tokenized.data.bigram)
...
bigrams <- raw.data %>% 
  ...
  summarise(unique_bigrams = n(), .by = medical_specialty)
knitr::kable(bigrams)
...
**Answer:**  
There are 1087 unique sentences in Surgery, 354 in Orthopedic, and 273 in Radiology.



```{r}
sentence.data <- analysis.data %>%
  tidytext::unnest_tokens(sentence, transcription, token = "sentences")

unique_sentences <- sentence.data %>%
  group_by(medical_specialty) %>%
...
```{r}
sent_counts <- raw.data %>%
  unnest_tokens(sentence, transcription, token = "sentences") %>% 
  distinct(medical_specialty, sentence) %>% 
  count(medical_specialty, name = "unique_sentences")
knitr::kable(sent_counts)
...
**Answer:**  
No, general-purpose lemmatizers usually miss medical terms and abbreviations. Medical text has a lot of specialized language that general models don't recognize.
...
**Answer:**  
Orthopedic and Surgery have very similar lemma frequencies, while Radiology is different. This matches our expectations since the first two both describe procedures and use similar words, while Radiology uses more imaging-specific terms.
...
```{r}
library(tidyr)
comparison_data <- lemma.freq %>%
  filter(medical_specialty %in% c("Orthopedic", "Radiology", "Surgery")) %>%
  pivot_wider(names_from = medical_specialty, values_from = proportion)

ggplot(comparison_data, aes(x = Orthopedic, y = Radiology)) +
  geom_point(aes(color = abs(Orthopedic - Radiology))) +
  geom_abline(color = "gray40", lty = 2) +
  facet_wrap(~ Surgery) +
  labs(x = "Orthopedic Proportion",
       y = "Radiology Proportion",
       title = "Orthopedic vs. Radiology Lemma Usage (faceted by Surgery)") +
  theme_minimal()

```
```{r}
wide_data <- lemmatized.data %>%
...
ggplot(wide_data, aes(x = Surgery, y = Radiology)) +
  geom_point(alpha = 0.2) +
  geom_abline(color = "gray40", lty = 2) +
  labs(x = "Surgery Proportion", y = "Radiology Proportion",
       title = "Surgery vs Radiology Lemma Proportions") +
  theme_minimal()
...
The scatter plot compares lemma usage in Surgery and Radiology. Most points fall below the diagonal, showing that many terms are more common in Surgery.
...
**Answer:**  
Yes, many top tf-idf lemmas are highly specific to each specialty, like "carotid artery" in Radiology or "steri strips" in Orthopedic.
...
example <- analysis.data %>%
  ...
  filter(str_detect(transcription, 'steri strips'))
...
This code shows an example of how the top lemma "steri strips" is used in context within an orthopedic or surgical note.

```{r}
analysis.data %>%
  select(medical_specialty, transcription) %>%
  filter(str_detect(transcription, "carotid artery")) %>%
  head(1)
...
```{r}
library(topicmodels)
emr.lda6 <- LDA(emr.dcm, k = 6, control = list(seed = 2025))

library(tidytext)
beta6 <- tidy(emr.lda6, matrix = "beta")
...
```
```{r}
lemma.counts.dtm <- lemmatized.data %>%
  group_by(note_id, lemma) %>%
  summarise(n = n(), .groups = "drop")

dtm <- tidytext::cast_dtm(lemma.counts.dtm, note_id, lemma, n)
emr.lda6 <- topicmodels::LDA(dtm, k = 6, control = list(seed = 42))
...
Many of the top terms in the 5-topic model still appear in the 6-topic LDA. With more topics, specialties are split into more detailed sub-topics, capturing finer differences within each specialty.
