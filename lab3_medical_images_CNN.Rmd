...
**Answer:** 
Pneumonia is an infection that causes inflammation in the air sacs of one or both lungs. These air sacs may fill with fluid, leading to symptoms like cough, fever, and difficulty breathing. Automatically identifying pneumonia from chest X-rays helps doctors make quicker and more accurate diagnoses, especially in busy clinics or regions with few radiologists. It can also help in early detection and better patient outcomes.
...
**Answer:** 
Yes, the filenames sometimes contain extra information, such as "bacteria" or "virus", which hints at the type of pneumonia. This makes prediction more challenging because a good model should only use the image content, not the filename, to make a diagnosis. If the filenames leak label information, the model might overfit and not generalize well to new data.
...
**Answer:** 
Yes, the images may vary in brightness, contrast, and size. Some images might be darker or lighter than others, or have different levels of sharpness. To help the model learn better, it is important to normalize pixel values and resize all images to the same dimensions.
...
**Answer:** 
If the dataset was not balanced, we could use techniques like:

Oversampling the minority class

Undersampling the majority class

Using class weighting during training

Applying data augmentation more to the minority class

These methods help the model learn from both classes more equally and avoid bias.
...
**Answer:** 
No, the model’s accuracy remains around 50% for both training and testing, which is the same as random guessing for a balanced binary dataset. This suggests the model is not learning meaningful patterns from the images. Possible reasons include a model that is too simple or too complex, not enough training data, or issues with preprocessing.
...
Use data augmentation to increase dataset variety

Try a deeper or more suitable CNN architecture
...
These steps can help the model learn better features and generalize to new data.
...
**Answer:** 
Data augmentation can introduce unrealistic or irrelevant variations if not done carefully, which may confuse the model. Over-augmentation can also make training slower and may not always help if the original data is already diverse.
...
ImageNet is a large visual database used for object recognition research. The 2012 challenge involved about 1.2 million images and 1,000 classes. Models trained on ImageNet have learned rich visual features that can be reused for other tasks, making transfer learning very effective, especially with limited data.
...
MobileNet is efficient and designed for fast, accurate image classification with fewer parameters. Using MobileNet allows us to take advantage of features learned from millions of images, improving our model’s accuracy on medical X-rays even with a small dataset.
...
MobileNet v2 has about 2.5 million parameters, which is much fewer than deeper networks like ResNet-50 (about 25 million) or VGG16 (about 138 million). Despite having fewer parameters, MobileNet still performs well, making it suitable for tasks with less data or limited computing power.
...
I tried to use torchvision's pretrained models (such as AlexNet, ResNet18, MobileNet) in R, but repeatedly encountered the error `Error in x$get_inherit() : attempt to apply non-function`. This is a known compatibility issue between the R torch/torchvision package and the underlying pre-trained model files, especially on Mac OSX.

Despite updating all packages, reinstalling, and restarting my R session, I was unable to resolve the error. All the image pre-processing and custom CNN model code ran successfully. In a real-world setting, I would switch to Python (PyTorch or Keras) or use Google Colab to run transfer learning. The failure is due to environment compatibility rather than code logic. All required steps have been attempted and documented.
